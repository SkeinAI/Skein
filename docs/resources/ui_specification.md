...
*   **品牌高亮主色 (`--skein-accent`)**：全局（浅色与深色模式下）一致采用高雅纯净的统一主题深蓝色 `#155aef`，确保最极致的品牌专注度、高清晰对比度以及低视觉压力。
*   **设计原则**：全局彻底摒弃高频刺眼的双色线性渐变背景，以自适应的纯净主题蓝色 `var(--skein-accent)` 代替，以实现高度沉静、专业、无视觉压力的界面观感。
...
*   **激活状态反馈**：当前选中助手应获得清晰的品牌蓝青色细微描边 (`1.5px solid var(--skein-accent)`) 以及半透明淡蓝色背景。
*   **头像背景**：选中助手的 Avatar 图标使用统一的主题纯色 `var(--skein-accent)` 作为底色。严禁混入任何以紫色/靛蓝色（如 `#6366f1`、`#8b5cf6`）为底座的杂色渐变，且彻底杜绝多余渐变以消除视觉压迫感。
