...
use skein_skills::context_modifier::ContextModifier;
use skein_skills::executor::{execute_fork, prepare_inline_content};
use skein_skills::hooks::{parse_skill_hooks, to_hook_defs};
use skein_skills::permissions::{SkillPermission, SkillPermissionChecker};
use skein_skills::types::{ExecutionContext, SkillMetadata};
...
impl Tool for SkillTool {
    ...
    fn context_modifier_for(&self, input: &serde_json::Value) -> Option<ContextModifier> {
        ...
        skein_skills::context_modifier::from_skill_metadata(skill)
    }
...
mod tests {
    use super::*;
    use skein_skills::permissions::SkillPermissionChecker;
    use skein_skills::types::{ExecutionContext, LoadChannel, SkillSource};
    use serde_json::json;
...
mod supplemental_tests {
    ...
    use skein_skills::permissions::SkillPermissionChecker;
    use skein_skills::types::{ExecutionContext, LoadChannel, SkillMetadata, SkillSource};
    ...
    async fn tc_13_8_full_variable_substitution_integration() {
        let mut skill = make_skill("my-skill", "Run ${skein_SKILL_DIR}/tool.sh $ARGUMENTS[0]");
