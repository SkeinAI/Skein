...
mod tests {
    ...
    impl Tool for MockDeferredTool {
        ...
        async fn execute(&self, input: serde_json::Value) -> skein_core::types::tool::ToolResult {
            if input.get("tasks").is_none() {
                return skein_core::types::tool::ToolResult {
                    content: "Missing or invalid 'tasks' array".to_string(),
                    is_error: true,
                };
            }
            skein_core::types::tool::ToolResult {
                content: "ok".to_string(),
                is_error: false,
            }
        }
        fn category(&self) -> skein_core::ipc_interface::events::ToolCategory {
            skein_core::ipc_interface::events::ToolCategory::Exec
        }
    }
    ...
    async fn execute_single_deferred_tool_error_missing_required_appends_hint() {
        ...
        let (result, _) = execute_single(
            ...
            &call,
            None,
            skein_core::context_compression::CompressionLevel::Off,
            false,
        )
        .await;
