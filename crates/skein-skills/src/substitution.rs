...
mod tests {
    ...
    #[test]
    fn test_skein_skill_dir_substitution() {
        let result = substitute_arguments(
            "dir=${skein_SKILL_DIR}",
            None,
            &[],
    ...
    #[test]
    fn test_skein_session_id_substitution() {
        let result = substitute_arguments(
            "sid=${skein_SESSION_ID}",
            None,
            &[],
            None,
            Some("sess-123"),
...
mod supplemental_tests {
    ...
    // -----------------------------------------------------------------------
    // TC-6.x: ${skein_SKILL_DIR} substitution
    // -----------------------------------------------------------------------
    ...
    fn tc_6_1_skill_dir_replaced() {
        let r = substitute_arguments(
            ...
            None,
            &[],
            Some("/home/user/.skein/skills/my-skill"),
            None,
        );
        assert_eq!(r, "cd /home/user/.skein/skills/my-skill");
    }

    #[test]
    fn tc_6_2_skill_dir_none_not_replaced() {
        // skill_root = None → ${skein_SKILL_DIR} stays unreplaced
        let r = substitute_arguments("cd ${skein_SKILL_DIR}", None, &[], None, None);
        assert_eq!(r, "cd ${skein_SKILL_DIR}");
    }

    #[test]
    fn tc_6_3_skill_dir_multiple_occurrences() {
        let r = substitute_arguments(
            "${skein_SKILL_DIR}/a and ${skein_SKILL_DIR}/b",
            None,
            &[],
            Some("/skills/foo"),
            None,
    ...
    // -----------------------------------------------------------------------
    // TC-7.x: ${skein_SESSION_ID} substitution
    // -----------------------------------------------------------------------

    #[test]
    fn tc_7_1_session_id_replaced() {
        let r = substitute_arguments(
            "Session: ${skein_SESSION_ID}",
            None,
            &[],
            None,
            Some("abc-123"),
    ...
    fn tc_7_2_session_id_none_not_replaced() {
        let r = substitute_arguments("Session: ${skein_SESSION_ID}", None, &[], None, None);
        assert_eq!(r, "Session: ${skein_SESSION_ID}");
    }
    ...
    fn tc_9_1_multiple_placeholder_types() {
        let r = substitute_arguments(
            "cd ${skein_SKILL_DIR} && run $ARGUMENTS[0] with $ARGUMENTS",
            Some("alpha beta"),
            &[],
            Some("/skills/foo"),
            None,
