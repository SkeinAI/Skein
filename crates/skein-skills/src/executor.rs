...
use skein_core::types::spawner::{ForkOverrides, Spawner, SubAgentConfig};
...
mod tests {
    ...
    async fn test_prepare_inline_substitutes_skill_dir() {
        let skill = make_skill("Dir: ${skein_SKILL_DIR}", Some("/skills/mine"));
        let result = prepare_inline_content(&skill, None, None, "/tmp")
            .await
            .unwrap();
    ...
    async fn test_prepare_inline_substitutes_session_id() {
        let skill = make_skill("Session: ${skein_SESSION_ID}", None);
...
mod phase7_tests {
    ...
    use skein_core::types::spawner::{ForkOverrides, Spawner, SubAgentConfig, SubAgentResult};
