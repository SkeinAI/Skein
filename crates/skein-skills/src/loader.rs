...
///
/// `extra_raw_dirs` are directories loaded directly (no `.skein/skills` suffix).
...
pub async fn load_all_skills(
    ...
    // 5. Extra raw dirs (direct folder import, no .skein/skills suffix)
    let futures: Vec<_> = extra_raw_dirs
        .iter()
        .filter(|d| d.is_dir())
        .map(|d| load_skills_from_dir(d, SkillSource::User, LoadChannel::Skills))
...
async fn load_skill_file(
    ...
    // used for ${skein_SKILL_DIR} variable substitution in skill content.
...
///
/// Examples:
/// - base=`<config_dir>/skein/skills`, target=`<config_dir>/skein/skills/db/migrate` → `"db:migrate"`
/// - base=`<config_dir>/skein/skills`, target=`<config_dir>/skein/skills/my-skill` → `"my-skill"`
